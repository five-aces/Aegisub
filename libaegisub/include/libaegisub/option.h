...
public:
	...
	Options(std::filesystem::path const& file, std::pair<const char *, size_t> default_config, const OptionSetting setting = NONE);

	template<size_t N>
	Options(std::filesystem::path const& file, const char (&default_config)[N], const OptionSetting setting = NONE)
