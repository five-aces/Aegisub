...
public:
	...
	MRUManager(std::filesystem::path const& config, std::pair<const char *, size_t> default_config, agi::Options *options = nullptr);

	template<size_t N>
	MRUManager(std::filesystem::path const& file, const char (&default_config)[N])
	...
	void Add(const char *key, std::filesystem::path const& entry);

	/// @brief Remove entry from the list.
	/// @param key List name
	/// @param entry Entry to add
	/// @exception MRUError thrown when an invalid key is used.
	void Remove(const char *key, std::filesystem::path const& entry);
	...
	/// @exception MRUError thrown when an invalid key is used.
	std::filesystem::path const& GetEntry(const char *key, const size_t entry);
