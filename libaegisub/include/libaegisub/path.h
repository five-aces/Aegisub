...
public:
	...
	std::filesystem::path Decode(std::string const& path) const;
	...
	std::filesystem::path MakeAbsolute(std::filesystem::path path, std::string const& token) const;
	...
	/// @throws InternalError if `token` is not a valid token name
	std::filesystem::path MakeRelative(std::filesystem::path const& path, std::string const& token) const;
	std::filesystem::path MakeRelative(std::filesystem::path const& path, const char *token) const { return MakeRelative(path, std::string(token)); }
	...
	std::filesystem::path MakeRelative(std::filesystem::path const& path, std::filesystem::path const& base) const;
	...
	std::string Encode(std::filesystem::path const& path) const;
	...
	/// @throws InternalError if `token` is not a valid token name
	void SetToken(const char *token_name, std::filesystem::path const& token_value);
};
